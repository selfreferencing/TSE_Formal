...
/-- Hurwitz stability: all eigenvalues have negative real part.
    ...
    Note: This definition requires Matrix.spectrum which may need 
    additional imports. If unavailable, treat Hurwitz as opaque. -/
def Hurwitz (J : Matrix ι ι ℝ) : Prop :=
  ∀ μ : ℂ, μ ∈ Matrix.spectrum ℂ (J.map (algebraMap ℝ ℂ)) → μ.re < 0
...
theorem uniform_certificate_bound (J0 J1 : Matrix ι ι ℝ) :
  ...
  simp only [CertSet, Set.eq_empty_iff_forall_not_mem, Set.mem_setOf_eq] at hempty
